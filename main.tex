...
\begin{abstract}
    The Minimum Scan Cover (MSC) problem schedules pairwise communication among a satellite swarm using directional antennas. Satellites are points in the Euclidean space, and alignment costs are proportional to the rotation angles.
    %
...
\begin{resumo}
    O problema da Cobertura Mínima por Varredura (MSC) escalona as comunicações par-a-par em enxames de satélites usando antenas direcionais. Satélites são pontos no espaço Euclidiano e os custos de alinhamento são proporcionais aos ângulos de rotação.
    %
...
%
Consequentemente, o tempo de comunicação não é instantâneo, mas dominado pelo direcionamento mecânico ou eletrônico de feixes para estabelecer \emph{links}.
%
...
%
Diferentemente de cenários de \emph{broadcasting}, em que a informação se propaga de uma fonte a todos os demais, o MSC requer apenas a satisfação de um conjunto prescrito de \emph{links} de comunicação, especificados pelas arestas de um grafo de entrada.
%
...
%
Especificamente, eles apresentaram uma \mbox{$\frac{9}{2}$-aproximação} para grafos bipartidos, que estenderam para uma \mbox{$O(\gamma)$-aproximação}, com $\gamma \ge 1$, quando $G$ é dado juntamente a uma $k$-coloração própria de seus vértices, em que $k \leq \chi(G)^\gamma$.
...
%
Para um inteiro não negativo $N$, usamos a notação $[N]:=\{0,1,\dots,N\}$.
%
...
Como, uma vez que todas as arestas incidentes a um vértice $p_i\in P$ tenham sido varridas, $p_i$ cessa a rotação de sua antena, os fatos acima nos permitem mapear cada cobertura por varredura $\cals$ de uma instância $(P, G)$ de MSC1D, com tempo de varredura no máximo $\pi N$, onde $N\in \N$, para uma única função $h\colon P \to \{0,1\}^{N+1}$.
%
...
%
Ainda assim, existe uma cobertura por varredura de tempo no máximo $\pi N$ se e somente se existe uma função ${h\colon P \to \{0,1\}^{N+1}}$ tal que, para cada aresta $p_ip_j\in E$, com $p_i<p_j$, existe um índice $m$ tal que ${h_m(p_i)=0}$ e ${h_m(p_j)=1}$.
%
De fato, o sentido direto segue da definição de $h$: no instante em que $p_i$ e $p_j$ se comunicam, a antena do ponto à esquerda aponta para a direção positiva e a antena do ponto à direita aponta para a direção negativa.
%
Para o sentido inverso, dada uma função $h$ com essa propriedade, definimos, para cada aresta $e=p_ip_j$ com $p_i<p_j$,
\[
    \cals_h(e)=\pi \cdot \min\{m\in [N]\mid h_m(p_i)=0 \text{ e } h_m(p_j)=1\}.
\]
%
Mostremos que $\cals_h$ é uma cobertura por varredura válida.
%
Considere duas arestas adjacentes $e$ e $f$ com vértice comum $p$.
%
Se os outros extremos de $e$ e $f$ estão do mesmo lado de $p$, então $\alpha(e,f)=0$.
%
Caso contrário, para varrer $e$ e $f$, a antena de $p$ deve apontar em direções opostas; portanto, os índices escolhidos pela definição de $\cals_h(e)$ e $\cals_h(f)$ não podem coincidir.
%
Como esses índices são inteiros, segue que $|\cals_h(e)-\cals_h(f)|\ge \pi=\alpha(e,f)$.

Esse mapeamento conecta o MSC1D a uma variante do clássico problema de $q$-Coloração, que pergunta se um dado grafo pode ser colorido propriamente com $q$ cores: cada vetor em $\{0,1\}^{N+1}$ pode ser interpretado como uma ``cor''.
%
Assim, podemos ver $h$ como uma coloração própria dos vértices de $G$, com cores em $\{0,1\}^{N+1}$, com o requisito adicional de que, para cada aresta $p_ip_j\in E$, com $p_i<p_j$, existe um índice $m$ tal que $h_m(p_i)=0$ e $h_m(p_j)=1$.

Com isso, podemos adaptar o algoritmo $\text{FPT}$ para $q$-Coloração~\cite{Cy15}, parametrizado pela largura arbórea $k$, que executa em tempo $q^k\cdot k^{O(1)}\cdot n$, para a versão de decisão do MSC1D, também parametrizada por $k$.
%
Precisamos apenas incorporar a restrição adicional sobre as cores atribuídas aos vértices adjacentes de $G$.
%
Podemos verificar se duas cores diferentes podem ser atribuídas a um par de nós vizinhos em tempo $N^{O(1)}$: basta testar se há uma coordenada $m$ em que a cor do vértice à esquerda tem valor $0$ e a cor do vértice à direita tem valor $1$.
%
Esse custo é pago nas verificações de compatibilidade das arestas e acrescenta apenas o fator $N^{O(1)}$ ao tempo de execução da programação dinâmica; portanto, essa modificação pode ser adicionada mantendo ainda um tempo de execução $\text{FPT}$.
%
...
%
Como é padrão na literatura, supomos que, junto com a instância de entrada, também recebemos uma decomposição arbórea \emph{nice} de largura $k$ com nós de introdução de aresta do grafo de entrada $G$ (consulte~\cite{Cy15} para uma definição de tal decomposição).
%
...
\begin{proof}
    Seja $I=(P, G)$ uma instância MSC2D dada juntamente com uma decomposição arbórea \emph{nice} de largura $k$, $\calt = {(T, \{X_t\}_{t\in V(T)})}$, com nós de introdução de aresta de $G$, e seja $r$ a raiz designada de $T$.
    %
    ...
    Usamos programação dinâmica baseada na decomposição arbórea \emph{nice} $\calt$ para computar uma cobertura ótima por varredura $\hat \cals$ para $\hat I$.
    %
    Para um nó $t\in V(T)$ na decomposição arbórea, sejam $P_t$ e $E_t$ os conjuntos de vértices e arestas, respectivamente, introduzidos na subárvore de $T$ enraizada em $t$, incluindo $t$ e todos os seus descendentes. Seja $G_t = (P_t, E_t)$ o subgrafo correspondente de $G$ e $F_t$ as arestas de $E_t$ que também estão presentes em $G[X_t]$.
    %
